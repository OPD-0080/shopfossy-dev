...
// CLOTH & SHOES START
    ...
    class Products {
        async getList() {
            var res = await fetch("json/main-categories/items-name-1/mainData.json");
            var data = await res.json();
            var desRes = await fetch("json/main-categories/items-name-1/description.json");
            var desData = await desRes.json();

            var list = data;
            var descriptions = desData;
            ...
            descriptions.map((item) => {
                ...
                var {tb_para_1} = item.tb_para_1;
                var {tb_para_2} = item.tb_para_2;
                var {tb_para_3} = item.tb_para_3;
                var {tb_para_4} = item.tb_para_4;
                var {tb_para_5} = item.tb_para_5;
                var {tb_para_6} = item.tb_para_6;
                var {tb_para_7} = item.tb_para_7;
                var {tb_para_8} = item.tb_para_8;
                var {tb_para_9} = item.tb_para_9;
                var {tb_para_10} = item.tb_para_10;
                var {tb_para_11} = item.tb_para_11;
                var {tb_para_12} = item.tb_para_12;
                var {tb_para_13} = item.tb_para_13;
                var {tb_para_14} = item.tb_para_14;
                var {tb_para_15} = item.tb_para_15;
                var {tb_det_1} = item.tb_det_1;
                var {tb_det_2} = item.tb_det_2;
                var {tb_det_3} = item.tb_det_3;
                var {tb_det_4} = item.tb_det_4;
                var {tb_det_5} = item.tb_det_5;
                var {tb_det_6} = item.tb_det_6;
                var {tb_det_7} = item.tb_det_7;
                var {tb_det_8} = item.tb_det_8;
                var {tb_det_9} = item.tb_det_9;
                var {tb_det_10} = item.tb_det_10;
                var {tb_det_11} = item.tb_det_11;
                var {tb_det_12} = item.tb_det_12;
                var {tb_det_13} = item.tb_det_13;
                var {tb_det_14} = item.tb_det_14;
                var {tb_det_15} = item.tb_det_15;
                ...
                return (
                    ...
                    tb_para_1, tb_para_2, tb_para_3, tb_para_4, tb_para_5, tb_para_6, tb_para_7, tb_para_8, tb_para_9, tb_para_10, tb_para_11, tb_para_12,
                    tb_para_13, tb_para_14, tb_para_15,
                    tb_det_1, tb_det_2, tb_det_3, tb_det_4, tb_det_5, tb_det_6, tb_det_7, tb_det_8, tb_det_9, tb_det_10, tb_det_11, tb_det_12,
                    tb_det_13, tb_det_14, tb_det_15
                );
            })
            //const list = await destructure_products_items(lists);
            //const descriptions = await destructure_Item_description(desArray);
    ...
    export class UI {
        ...
        cartLogic() {
            ...
            clearBtn.onclick = () => {
                this.clearCart();
                emptyCartAlert.classList.add("collapse");  // show cart Alert
            };
            // clearing all cart items start
